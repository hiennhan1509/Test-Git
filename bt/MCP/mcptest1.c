...
void EN_DIS_REDLED(unsigned int *GPIOD_PDOR);
...
int main(void)
{	
	unsigned int* GPIOD_PTOR = (unsigned int*) (0X400FF0C0 + 0X0C);
	f_BlinkRedLed();
	while(1)
	{
		//f_delay(1000);
		f_BitMaskRevision(GPIOD_PTOR,15,SET_BIT); /*Set Toggle Register for Port D - Pin 15*/
	}
}
...
{
	unsigned int* PCC_PORTC = (unsigned int*) (PCC + PORTC);
	unsigned int* PORT_PCR12 = (unsigned int*) (BASE_PORTC + 0X30);
	unsigned int* GPIOC_PDDR = (unsigned int*) (GPIOC + PDDR);
	unsigned int* GPIOC_PDIR = (unsigned int*) (GPIOC + PDIR);
	/*Config GPIO*/
	f_ConfigureGPIO(PCC_PORTC,PORT_PCR12);
	/*Set Input*/
	f_BitMaskRevision(GPIOC_PDDR,12,CLEAR_BIT); /*SET PORT D PIN 13 as Input*/
	if(ReadBit(GPIOC_PDIR,12) == 1) 
	{
		f_BitMaskRevision(GPIOD_PDOR,15,CLEAR_BIT);
		f_BitMaskRevision(GPIOD_PDOR,0,CLEAR_BIT);
		f_BitMaskRevision(GPIOD_PDOR,16,CLEAR_BIT);
	}
}
...
void DIS_RGBLED(unsigned int *GPIOD_PDOR)
{
	...
	unsigned int* GPIOC_PDDR = (unsigned int*) (GPIOC + PDDR);
	unsigned int* GPIOC_PDIR = (unsigned int*) (GPIOC + PDIR);
	unsigned char nPosition = 13;
	/*Config GPIO*/
	f_ConfigureGPIO(PCC_PORTC,PORT_PCR13);
	/*Set Input*/
	f_BitMaskRevision(GPIOC_PDDR,nPosition,CLEAR_BIT); /*SET PORT D PIN 13 as Input*/
	if(ReadBit(GPIOC_PDIR,nPosition) == 1)
	{
		f_BitMaskRevision(GPIOD_PDOR,15,SET_BIT);
		f_BitMaskRevision(GPIOD_PDOR,0,SET_BIT);
		f_BitMaskRevision(GPIOD_PDOR,16,SET_BIT);
	}
}
